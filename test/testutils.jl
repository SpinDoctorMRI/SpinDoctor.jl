...
get_setup(S::Type{PlateSetup{T}}) where {T} = S(;
    ...
    κ = (;
        interfaces = [1e-4 for _ = 1:4-1],
        boundaries = [0.0 for _ = 1:4],
    ),
    γ = 2.67513e-4,
)
...
get_coeffs(setup::PlateSetup) = coefficients(setup)

get_coeffs(setup::CylinderSetup) = coefficients(setup)

get_coeffs(setup::SphereSetup) = coefficients(setup)

get_coeffs(setup::NeuronSetup) = coefficients(setup)
