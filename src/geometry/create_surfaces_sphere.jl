...
function create_surfaces_sphere(cells, setup::Setup)
    ...
    @unpack ncell, rmin, rmax, include_in, in_ratio, include_ecs, ecs_shape, ecs_ratio = setup.geometry
    ...
    if include_ecs
        ...
        elseif ecs_shape == "convex_hull"
            ...
            [facets_ecs[i, j] = findfirst(facets_ecs[i, j] .== ecs_hull.vertices) for j = 1:length(ecs_hull.simplices), i = 1:3]
